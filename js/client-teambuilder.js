	...
	exports.TeambuilderRoom = exports.Room.extend({
		...
		clipboardInnerHTML: function () {
			...
			for (var i = 0; i < this.clipboardCount(); i++) {
				...
				buf += '<div class="section"><span class="icon" style="' + Dex.getPokemonIcon(set, false, this.curTeam.mod) + '"></span>';
		...
		chooseMove: function (moveName, resetSpeed) {
			...
			for (var i = 0; i < moves.length; ++i) {
				...
				var move = 0;
				if (this.curTeam.mod) move = Dex.mod(this.curTeam.mod).getMove(moves[i]);
				else move = Dex.forGen(this.curTeam.gen).getMove(moves[i]);
		...
		setPokemon: function (val, selectNext) {
			var set = this.curSet;
			var species = 0;
			if (this.curTeam.mod) species = Dex.mod(this.curTeam.mod).getSpecies(val);
			else species = Dex.forGen(this.curTeam.gen).getSpecies(val);
	...
	var MoveSetPopup = exports.MoveSetPopup = Popup.extend({
		initialize: function (data) {
			...
			for (var i = 0; i < data.team.length; i++) {
				...
				buf += '<li' + (i === data.i ? ' style="opacity:.3"' : ' style="opacity:.6"') + '><span class="picon" style="display:inline-block;vertical-align:middle;' + Dex.getPokemonIcon(set, false, this.curTeam.mod) + '"></span> ' + BattleLog.escapeHTML(set.name || set.species) + '</li>';
			}
			if (i !== data.i && i !== data.i + 1) {
				buf += '<li><button name="moveHere" value="' + i + '"><i class="fa fa-arrow-right"></i> Move here</button></li>';
			}
