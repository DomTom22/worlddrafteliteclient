...
class Battle {
	...
	*/
	...
	nextStep() {
		if (this.atQueueEnd || (this.paused && this.turn >= 0)) return;
		...
		while (!animations) {
			...
			if (this.paused && this.turn >= 0) {
				// initial Play button, no team preview
				this.scene.pause();
				return;
			}
			if (this.atQueueEnd) return;
